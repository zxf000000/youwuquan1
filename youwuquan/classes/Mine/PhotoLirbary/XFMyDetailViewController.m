...
@interface XFMyDetailViewController () <ASTableDelegate,ASTableDataSource,XFStatusCellNodeDelegate,XFFindCellDelegate,AVAudioPlayerDelegate>
...
@implementation XFMyDetailViewController
...
- (void)tableNode:(ASTableNode *)tableNode didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row > 0) {
        ...
        statusDetailVC.status = self.datas[indexPath.row - 1];
        statusDetailVC.deleteSUccessBlock = ^{
          
            [self.datas removeObject:self.datas[indexPath.row - 1]];
            self.isChanged = YES;

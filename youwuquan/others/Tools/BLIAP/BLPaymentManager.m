...
- (void)transactionFailed:(SKPaymentTransaction *)transaction {
    ...
    if(transaction.error.code != SKErrorPaymentCancelled) {
        ...
        self.HUD.mode = MBProgressHUDModeText;
        self.HUD.label.text = transaction.error.localizedDescription;
        [self.HUD hideAnimated:YES afterDelay:0.5];
...
- (void)finishATransationWithIndentifier:(NSString *)transactionIdentifier {
    ...
    if (!targetTransaction) {
        ...
        NSLog(@"%@--------",targetTransaction);
        
        // 调用充值接口
        self.HUD.mode = MBProgressHUDModeText;
        self.HUD.label.text = @"验证成功,开始充值";
        [self.HUD hideAnimated:YES afterDelay:0.5];
       
        self.checkSuccessWithId(targetTransaction.payment.productIdentifier);

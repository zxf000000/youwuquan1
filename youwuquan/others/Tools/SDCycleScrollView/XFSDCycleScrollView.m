...
#import "XFSDCollectionViewCell.h"
...
NSString * const ID = @"XFSDCycleScrollViewCell";

@interface XFSDCycleScrollView () <UICollectionViewDataSource, UICollectionViewDelegate>
...
@implementation XFSDCycleScrollView
...
- (void)initialization
{
    _pageControlAliment = XFSDCycleScrollViewPageContolAlimentCenter;
    ...
    _pageControlStyle = XFSDCycleScrollViewPageContolStyleClassic;
...
+ (instancetype)xfcycleScrollViewWithFrame:(CGRect)frame imageNamesGroup:(NSArray *)imageNamesGroup
{
    XFSDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    cycleScrollView.localizationImageNamesGroup = [NSMutableArray arrayWithArray:imageNamesGroup];
    return cycleScrollView;
}

+ (instancetype)xfcycleScrollViewWithFrame:(CGRect)frame shouldInfiniteLoop:(BOOL)infiniteLoop imageNamesGroup:(NSArray *)imageNamesGroup
{
    XFSDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
+ (instancetype)xfcycleScrollViewWithFrame:(CGRect)frame imageURLStringsGroup:(NSArray *)imageURLsGroup
{
    XFSDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
+ (instancetype)xfcycleScrollViewWithFrame:(CGRect)frame delegate:(id<XFSDCycleScrollViewDelegate>)delegate placeholderImage:(UIImage *)placeholderImage
{
    XFSDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
- (void)setupMainView
{
    ...
    [mainView registerClass:[XFSDCollectionViewCell class] forCellWithReuseIdentifier:ID];
...
- (void)setDelegate:(id<XFSDCycleScrollViewDelegate>)delegate
{
    _delegate = delegate;
...
- (void)setCurrentPageDotImage:(UIImage *)currentPageDotImage
{
    ...
    if (self.pageControlStyle != XFSDCycleScrollViewPageContolStyleAnimated) {
        self.pageControlStyle = XFSDCycleScrollViewPageContolStyleAnimated;
    }
...
- (void)setPageControlStyle:(XFSDCycleScrollViewPageContolStyle)pageControlStyle
{
...
- (void)setupPageControl
{
    ...
    switch (self.pageControlStyle) {
        case XFSDCycleScrollViewPageContolStyleAnimated:
        {
        ...
        case XFSDCycleScrollViewPageContolStyleClassic:
        {
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)layoutSubviews
{
    ...
    _flowLayout.itemSize = CGSizeMake(self.frame.size.width - 28, self.frame.size.height);
    ...
    if (self.pageControlAliment == XFSDCycleScrollViewPageContolAlimentRight) {
...
{
    XFSDCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ID forIndexPath:indexPath];
...
{
    if ([self.delegate respondsToSelector:@selector(xfcycleScrollView:didSelectItemAtIndex:)]) {
        [self.delegate xfcycleScrollView:self didSelectItemAtIndex:[self pageControlIndexWithCurrentCellIndex:indexPath.item]];
    }
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    if ([self.delegate respondsToSelector:@selector(xfcycleScrollView:didScrollToIndex:)]) {
        [self.delegate xfcycleScrollView:self didScrollToIndex:indexOnPageControl];

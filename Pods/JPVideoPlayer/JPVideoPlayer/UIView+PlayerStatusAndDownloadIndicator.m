...
- (void)layoutProgressViewForPortrait:(UIView *)progressView{
    CGFloat progressViewY = self.frame.size.height - 17;
    ...
    progressView.frame = CGRectMake(80, progressViewY, self.frame.size.width - 160, JPVideoPlayerLayerFrameY);
}
...
- (void)layoutProgressViewForLandscape:(UIView *)progressView{
    ...
    CGFloat progressViewY = hei - 23;
    if ([self.jp_videoPlayerDelegate respondsToSelector:@selector(shouldProgressViewOnTop)] && [self.jp_videoPlayerDelegate shouldProgressViewOnTop]) {
        progressViewY = 0;
    }
    progressView.frame = CGRectMake(80, progressViewY, width - 160, hei);
}
